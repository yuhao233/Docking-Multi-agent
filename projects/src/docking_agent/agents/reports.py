...
`parse_json_object` 校验必需键、失败重试一次」（`tools/dispatch._invoke_checked`）——
...
class PropertyReport(_ExtraAllowed):
    """分子属性评估 Agent 的输出。"""
...
class PocketReport(_ExtraAllowed):
    """口袋分析 Agent 的输出。"""
...
class DockingReport(_ExtraAllowed):
...
class BindingReport(_ExtraAllowed):
    """结合模式检测 Agent 的输出。"""
...
#: 角色 → 结构化输出模型（与 `tools/dispatch._REQUIRED_KEYS` 的关键字段一一对应）
