...
| **两种界面（同一后端契约）** | **简易模式** `/`（**默认首页**，别名 `/simple`）：仅「任务描述 + 筛选结果」两块，不暴露参数控件（质子化态/搜索强度/位点盒由默认值与受理层自动规划），结果区给前 5 名 + 关键指标 + 报告/CSV/整包链接；刷新后结果区置空，往次结果由「历史运行」显式载入。**高级模式** `/advanced`：参数表单、历史检索、报告全文、中间数据与设置页。两套界面均走标准 Agent Protocol（`/threads/{tid}/runs/stream`）与同一批 `/api/*` 产物接口 |
...
| **交互口径：执行细节自主处理** | 需要用户决定的只有影响对接本身的三类问题（受体不可用/歧义、多组分分子的代表结构、共晶配体是否作对照）。其余按执行细节处理：达到递归上限自动放宽（`RECURSION_LIMIT` ×2 ×4，`RECURSION_LIMIT_MAX` 默认 480 封顶）并从 checkpoint 续跑；到顶由协调 Agent 用现有结果收尾并照常产出报告，执行类失败（`GRAPH_RECURSION_LIMIT`、工具异常）不向用户上报为错误 |
