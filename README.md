...
| 检查 | 内容 |
| --- | --- |
| `pytest` | 计算内核、智能体契约、报告版式、接口协议、步数预算与工具调用序列自愈等 800 余项（全量 811 通过），多数可离线运行 |
| `scripts/lint_local.py` | 静态规范：返回类型、静默异常、裸环境变量、文件规模 |
| `scripts/check.sh --static` | 五道静态门禁一次跑完：lint、ruff、mypy、界面契约（`check_web.py`，239 项）、安全姿态 |
| `scripts/check_web.py` | 界面静态校验：DOM 契约、设计令牌、外部依赖、两套界面与候选交互契约 |
| `scripts/ui_e2e.js` | DOM 级端到端：对话、参数、结果、设置、协议帧、两套界面（230 项） |
| `scripts/browser_check.py` | 真实浏览器（85 项）：协议请求形状、Markdown 渲染、思考折叠、候选点选时机与运行态、报告版式 |
