...
| 界面 | 地址 | 适用场景 | 界面内容 |
| --- | --- | --- | --- |
| **简易模式** | `/`（**默认首页**，别名 `/simple`） | 仅需一次筛选结论 | 「任务描述 + 筛选结果」两块；不暴露参数控件，质子化态、搜索强度与位点盒由默认值与受理层自动规划；结果区列出前几名、关键指标与报告 / CSV / 整包链接 |
| **高级模式** | `/advanced` | 需要显式参数、中间数据与设置 | 三栏工作台：参数表单、对话与结果、运行详情（编排时间轴 / 阶段日志 / 工具轨迹 / 实时逐分子）；另有历史检索、报告全文、中间数据与设置页 |
...
执行细节由系统处理；需要用户裁决的只有**影响对接本身**的问题。

| 类别 | 触发条件 | 系统行为 |
| --- | --- | --- |
| 影响对接 | 受体不可用，或存在多个同等合理的候选；多组分分子（如代森锰锌）的代表结构取法；共晶配体是否作阳性对照 | 置 `needs_user_input`，**不执行计算**，下发候选项由用户裁决；所选结果作为请求字段在同一会话内续跑 |
| 执行细节 | 工具或模型调用失败；达到递归上限；同一问题被重复生成；页面刷新 | 自动重试；自动放宽上限（120 → 240 → 480）并从检查点续跑，到顶用现有结果收尾；同一问题只下发一次；刷新后结果区置空，往次结果由历史运行载回 |

运行状态区分「等待用户选择」`needs_user_input`（`[ ASK ]`）与「未受理、零计算」`no_op`（`[ SKIP ]`）。
...
一个协调 Agent 掌握全部工具，负责任务拆分、结果汇总与报告定制；四个子 Agent 分别承担口袋分析、
性质评估、对接执行与结合模式检测，按无状态执行器设计，每次调用使用独立线程。执行治理作用于每次模型调用：
中间件负责重试、模型与工具调用限额、长对话摘要、工具调用序列自愈；步数预算在达到递归上限时自动放宽
并从检查点续跑，到顶由协调 Agent 用现有结果收尾。跨步骤数据分两路交接：受体、位点盒、分子库标识、
阳性对照等小状态写入共享黑板；分子清单、性质与对接明细落盘为运行产物，按文件路径交接，
因此上万条明细不进入模型上下文。
...
用户输入先经受理层解析为结构化任务规约，给出执行、请求补充或拒绝的决策；后两种情形不调用计算工具，
因而不产生空报告。随后协调 Agent 按规约分发工具，计算完成后明细落盘、摘要回传。界面持续接收
文本增量、阶段进展与领域事件；模型推理单独成轨（折叠进「思考」块，不进入正文），需要用户裁决的候选
在本轮输出结束后下发。达到递归上限时自动放宽并从检查点续跑，到顶由协调 Agent 用现有结果收尾。
...
打开服务地址即是简易模式：输入任务描述（例如"筛选这 20 个分子对 EGFR 的结合"或"把拟南芥 ROS1 与
代森锰锌对接"）后提交。分子与受体可直接写名称（由在线数据库检索），也可上传文件；无需配置参数。
运行中可随时停止；完成后结果区给出推荐分子、关键指标与产物链接。
...
└── messages_log.json   模型可见消息日志（角色/工具/长度/开头 400 字，用于"气泡里说了什么"的事后核对）
```
...
| 检查 | 内容 |
| --- | --- |
| `pytest` | 计算内核、智能体契约、受理与继承、报告版式与 golden、接口协议、候选生命周期、工具调用序列自愈、步数预算、身份去重、外部引擎执行适配、默认引擎解析、大环配体与大库漏斗、阻断式询问与续跑、消息日志、正文数据折叠、安全边界、运行记录损坏与保留策略等，本机全量 **859 项**通过（多数用例可离线运行） |
| `projects/scripts/check.sh --static` | 五道静态门禁一次跑完：lint（手写 AST 检查 + 基线棘轮）、ruff、mypy（渐进白名单）、界面契约（`check_web.py`，239 项）、安全姿态 |
| `projects/scripts/check.sh --fast` | 与 CI 等价的离线快跑：跳过需要本机引擎的用例（606 通过 / 253 跳过，约 40 秒） |
| `projects/scripts/ui_e2e.js` | DOM 级端到端（241 项，jsdom + 真实后端）：对话、参数、结果、设置、协议帧、两套界面、正文数据折叠、门禁自清理 |
