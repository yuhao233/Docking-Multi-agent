...
| 检查 | 内容 |
| --- | --- |
| `pytest` | 计算内核、智能体契约、受理与继承、报告版式与 golden、接口协议、候选生命周期、工具调用序列自愈、步数预算、身份去重、外部引擎执行适配、默认引擎解析、大环配体与大库漏斗、阻断式询问与续跑、消息日志、正文数据折叠、安全边界、运行记录损坏与保留策略等，本机全量 **857 项**通过（多数用例可离线运行） |
| `projects/scripts/check.sh --static` | 五道静态门禁一次跑完：lint（手写 AST 检查 + 基线棘轮）、ruff、mypy（渐进白名单）、界面契约（`check_web.py`，239 项）、安全姿态 |
| `projects/scripts/check.sh --fast` | 与 CI 等价的离线快跑：跳过需要本机引擎的用例（604 通过 / 253 跳过，约 40 秒） |
